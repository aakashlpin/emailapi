...
import { AuthUserInfoContext, useFirebaseAuth } from '~/src/firebase/hooks';
import { createAuthUser, createAuthUserInfo } from '~/src/firebase/user';
...
export default (ComposedComponent) => {
  const WithAuthUserComp = (props) => {
    ...
    const { AuthUser: AuthUserFromSession, token, uid } = AuthUserInfo;
    const AuthUser = AuthUserFromClient || AuthUserFromSession || null;

    return (
      <AuthUserInfoContext.Provider value={{ AuthUser, token, uid }}>
